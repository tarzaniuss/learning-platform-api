...
from app.schemas.enrollment import Enrollment, EnrollmentCreate, LessonCompletionCreate, LessonCompletion
...
def enroll_in_course(
    ...
    course = db.query(CourseModel).filter(
        CourseModel.id == enrollment_data.course_id
    ).first()
...
def complete_lesson(
    ...
    lesson = db.query(LessonModel).filter(
        LessonModel.id == completion_data.lesson_id
    ).first()
    ...
    existing = db.query(LessonCompletionModel).filter(
    ...
    completed_lessons = db.query(LessonCompletionModel).join(
        LessonModel
    ).filter(
        LessonCompletionModel.user_id == current_user.id,
        LessonModel.course_id == lesson.course_id
